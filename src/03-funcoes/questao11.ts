...
export function lerTemperatura(): number {
    let entrada = prompt("Digite a temperatura: ")
    return Number(entrada)
}

export function verificarAlertaTemperatura(temperatura: number): boolean {
    return temperatura < 10 || temperatura > 30
}

export function emitirMensagemAlerta(statusAlerta: boolean): void {
    if (statusAlerta) {
...
export function questao11(): void {
    ...
    let alerta = verificarAlertaTemperatura(temperatura)
